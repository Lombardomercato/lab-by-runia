...
const initInteractiveHighlights = () => {
  ...
  highlights.forEach((highlight) => {
    ...
    const inertiaLerp = Number.parseFloat(styles.getPropertyValue('--highlight-inertia')) || 0.09;
    const drag = Number.parseFloat(styles.getPropertyValue('--highlight-drag')) || 0.72;
    ...
    const render = () => {
      ...
      const adaptiveFollow = Math.min(0.2, followLerp + distance * 0.00065);
      const fadeLerp = state.targetAlpha > state.alpha ? fadeLerpIn : fadeLerpOut;

      state.vx += dx * adaptiveFollow;
      state.vy += dy * adaptiveFollow;
      state.vx *= drag;
      state.vy *= drag;
      state.x += state.vx + dx * inertiaLerp;
      state.y += state.vy + dy * inertiaLerp;
      ...
      if (
        ...
        || Math.abs(state.ty - state.y) > settleThreshold
